...
export function KardexPage() {
  ...
  const columns: Column<MovementRow>[] = [
    { key: "d", header: "Fecha", className: "whitespace-nowrap", render: (m) => formatDateTime(m.created_at) },
    ...
    { key: "doc", header: "Documento", className: "whitespace-nowrap", render: (m) => <Link to={`/movimientos/${m.doc_id}`} onClick={(e) => e.stopPropagation()} className="text-brand hover:underline">{kindLabels[m.kind]} #{m.doc_number}</Link> },
    { key: "r", header: "Motivo", render: (m) => <span className="inline-flex items-center gap-1.5"><Badge tone={kindTone[m.kind]}>{reasonLabels[m.reason]}</Badge>{m.voided_at && <Badge tone="critical">Anulado</Badge>}</span> },
    { key: "x", header: "Detalle", render: (m) => <span className="text-ink-secondary">{[m.reference, m.party, m.user_name].filter(Boolean).join(" · ") || "—"}</span> },
    ...
    ...(isAdmin ? [{ key: "c", header: "Costo u.", align: "right" as const, className: "whitespace-nowrap tabular-nums text-ink-secondary", render: (m: MovementRow) => formatMoney(m.unit_cost) }] : []),
    ...
    <Page icon={ClipboardList} title="Kardex" width="full" subtitle="Movimientos de stock en unidad base, del más reciente al más antiguo.">
      <div className="mb-3 flex flex-wrap gap-2">
        <Select hideLabel label="Producto" value={productId} onChange={(e) => { setParams(e.target.value ? { producto: e.target.value } : {}); setPage(1); }} placeholder="Todos los productos"
          options={products.map((p) => ({ value: p.id, label: `${p.code} · ${productLabel(p)}` }))} className="w-96" />
        <Select hideLabel label="Tipo" value={kind} onChange={(e) => { setKind(e.target.value); setPage(1); }} placeholder="Entradas, salidas y ajustes"
          options={Object.entries(kindLabels).map(([value, label]) => ({ value, label: `${label}s` }))} className="w-56" />
      </div>
