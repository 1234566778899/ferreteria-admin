...
export function CountPage() {
  ...
  return (
    <Page icon={ClipboardCheck} title="Conteo físico" width="full"
      actions={<Button variant="primary" disabled={entered.length === 0} onClick={() => setConfirming(true)}>Aplicar<span className="max-sm:hidden">&nbsp;conteo</span>&nbsp;({diffs.length}<span className="max-sm:hidden">&nbsp;diferencias</span>)</Button>}>
      ...
      <div className="my-3 flex flex-wrap items-center gap-2">
        <Select hideLabel label="Categoría" value={category} onChange={(e) => setCategory(e.target.value)} placeholder="Todas las categorías" className="min-w-0 flex-1 sm:w-56 sm:flex-none" options={categories.map((c) => ({ value: c.id, label: c.name }))} />
        <Select hideLabel label="Ubicación" value={location} onChange={(e) => setLocation(e.target.value)} placeholder="Todas las ubicaciones" className="min-w-0 flex-1 sm:w-48 sm:flex-none" options={locations.map((l) => ({ value: l, label: l }))} />
        <input type="search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar producto" aria-label="Buscar producto"
          className="h-[30px] w-full rounded-[10px] sm:w-[280px] bg-white px-3 shadow-field outline-none focus:shadow-[0_0_0_2px_var(--color-brand)]" />
        <span className="text-ink-secondary sm:ml-auto">{entered.length} contados · {rows.length} en la lista</span>
      </div>
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="h-9 border-b border-border bg-surface-muted text-[12px] text-ink-secondary">
              <th className="pl-4 max-md:hidden">Ubicación</th><th className="px-3 max-md:pl-4">Producto</th><th className="px-3 text-right">En sistema</th><th className="px-3 text-right sm:w-40">Contado</th><th className="pr-4 pl-3 text-right">Diferencia</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((p) => {
              ...
              return (
                <tr key={p.id} className={cn("h-11 border-b border-border last:border-0", diff !== null && diff !== 0 && "bg-warning-soft/40")}>
                  <td className="pl-4 whitespace-nowrap text-ink-secondary max-md:hidden">{p.location ?? "—"}</td>
                  <td className="min-w-[160px] px-3 py-1.5 max-md:pl-4">
                    <span className="mr-2 font-mono text-[12px] text-ink-tertiary">{p.code}</span><span className="font-[550]">{productLabel(p)}</span>
                    {p.location && <span className="block text-[12px] text-ink-secondary md:hidden">{p.location}</span>}
                  </td>
                  <td className="px-3 text-right tabular-nums whitespace-nowrap">{formatQty(p.stock, p.unit)}</td>
                  <td className="px-3 text-right">
                    <input type="number" min="0" step="any" inputMode="decimal" value={v ?? ""} aria-label={`Contado de ${p.name}`}
                      ...
                      className="h-8 w-20 rounded-[8px] bg-white px-2 text-right sm:w-32 tabular-nums shadow-field outline-none focus:shadow-[0_0_0_2px_var(--color-brand)]" />
                  </td>
                  <td className={cn("pr-4 pl-3 text-right font-[550] tabular-nums whitespace-nowrap", diff === null ? "text-ink-tertiary" : diff === 0 ? "text-success" : diff > 0 ? "text-info" : "text-critical-strong")}>
